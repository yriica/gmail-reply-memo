...
### Changed
- **エディタの刷新**
  - Toast UI Editorから軽量なテキストエリアベースのエディタに変更
  - 依存関係を削除し、より安定した動作を実現
  - Googleデザインスタイルに準拠したシンプルなUI

### Added
- **Markdownリスト自動継続機能**
  - 箇条書きリスト（`*`, `-`, `+`）でEnterを押すと自動的に次の項目マーカーを挿入
  - 番号付きリスト（`1.`, `2.`など）で自動的に番号をインクリメント
  - チェックリスト（`- [ ]`, `- [x]`）の自動継続に対応
  - インデントの保持に対応
  - 空の項目でEnterを押すとリストを終了
...
- **IME対応の改善**
  - 日本語入力時の変換確定とEnterキーを適切に区別
  - `isComposing`チェックによる変換中の処理スキップ

### Improved
- **メモエリアの最適化**
  - 初期高さを3行分（70px）に縮小し、よりコンパクトに
  - 最大高さ（400px）を設定し、長文はスクロール対応
  - 垂直方向にリサイズ可能（`resize: vertical`）
  
- **UIの改善**
  - 枠線を削除し、Googleのデザインに統一
  - フォーカス時の背景色変更でより直感的な操作感
  - プレースホルダーをシンプルに変更

### Removed
- **Toast UI Editorの削除**
  - 外部ライブラリへの依存を削除
  - `libs/toastui-editor.js`、`libs/toastui-editor.css`を削除
  - WYSIWYGエディタから純粋なMarkdownテキストエリアへ移行
  - ファイルサイズの大幅な削減

### Fixed
- 日本語入力時に「* 有休」が「有休有休」となるバグを修正
- メモエリアの青い枠線が不自然に表示される問題を修正
- メモエリアが入力できなくなる問題を修正

### Technical
- IME変換検出ロジックの追加（`e.isComposing`, `keyCode === 229`）
- 正規表現によるMarkdownパターンマッチング
- フォールバック処理の簡素化とコードの保守性向上
